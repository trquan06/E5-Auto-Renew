...
Prepare a clean v2.0.0 repository with safe first-run behavior, delegated OAuth, a local multilingual SPA, reproducible containers, automated security checks, and complete operator documentation. Publication remains gated on Git/history, CI, GHCR, and clean-clone verification.
...
5. Add a non-root multi-stage Dockerfile, image/build Compose files, Portainer examples, health checks, OCI labels, GitHub CI/release workflow definitions, security scan definitions, and Dependabot configuration.
...
- Legacy repository-owner placeholders are replaced before public publishing.

Items above are release targets, not evidence that GitHub-hosted jobs or published artifacts have passed. See `E5_Auto_Renew_GitHub_Release_CHECKLIST.txt` for verified status and explicit `NOT VERIFIED` gates.
